...
const handleViewPDF = (TypedArray: Uint8Array) => {
  ...
  loadingTask.promise.then(
    async (pdf) => {
      ...
      const scale = scales[window.devicePixelRatio as keyof typeof scales] || defaultScale
...
const initSignatureByUrl = (url: string) => {
  ...
  div.addEventListener('mousedown', (event: Event) => {
    ...
    if (div.classList.contains('signature--selected') && target?.tagName === 'IMG') {
      startMouseMove()
    }
  })
...
const onMouseMove = (e: MouseEvent) => {
  ...
  const positionY = e.pageY - (e?.currentTarget as HTMLElement)?.getBoundingClientRect()?.top
  ...
  <div class="p-1 lg:p-4 bg-[#f0f3f5]">
    <div class="mx-auto">
      <div class="mb-4 flex items-center gap-4">
        <input type="file" accept="application/pdf" ref="inputFilePdf" @change="onChangeFilePdf" />

        <!-- upload image signature -->
        <button class="p-1 bg-white border border-solid rounded" type="button" @click="inputFileImage?.click()">
          Upload image signature
        </button>
        <input class="hidden" type="file" accept="image/*" ref="inputFileImage" @change="onChangeImageSignature" />

        <button class="p-1 bg-white border border-solid rounded" type="button" @click="toggleSignaturePad">
          Toggle signature pad
        </button>

        <button class="p-1 bg-white border border-solid rounded" type="button" @click="handleExport">
          Export Pdf
        </button>
      </div>
      ...
      <div class="p-1 lg:p-4 max-w-[1024px] mx-auto text-center">
        <div :id="ID_PDF_VIEWER" class="relative overflow-hidden" @click="dropSignature"></div>
      </div>
    </div>
  </div>
</template>
